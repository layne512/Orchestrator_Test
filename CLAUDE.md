...
9. **Spec-authoring grep-verification (SPEC_LESSONS L-002 + L-003 enforcement).** When authoring or editing any task spec for peptide-website (anything under `peptide-mvp/specs/`), every cited file path, helper / function name, signature, route path, table name, and string literal MUST be confirmed via `rg` / `cat` against the actual peptide-website checkout at the spec's anchor SHA. If the target checkout is NOT accessible from the current sandbox (no `git clone Layne512/peptide-website` available, no `GITHUB_TOKEN` set, no mounted volume), specs may be written ONLY to `peptide-mvp/specs/drafts/` — NEVER to `peptide-mvp/specs/W*/`. The `# unverified — grep before dispatch` warning-sticker pattern (L-002 layer 1) is RETIRED and FORBIDDEN. Specs containing that marker outside `drafts/` must be moved to `drafts/` immediately. Pre-commit hook in `.husky/pre-commit` runs `scripts/validate-spec.sh` against every staged spec under `peptide-mvp/specs/W*/` and refuses the commit on any failure. Bypassing the hook with `--no-verify` is itself a hard-rule violation.
